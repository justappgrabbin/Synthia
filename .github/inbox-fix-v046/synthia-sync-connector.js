(() => {
  ...
  async function decide(event) {
    ...
    try {
      const requestId = notice?.payload?.request_id;
      const relatedIds = [...new Set((requestId
        ? notices.filter(n => n?.payload?.request_id === requestId)
        : [notice]).map(n => n.notification_id).filter(Boolean))];
      ...
      // A connection request is mirrored to both "synthia" and "adaya". Mark every
      // local notification row for the same request read so a resolved request cannot
      // reappear and then fail with "not found" on a second approval attempt.
      await Promise.allSettled(relatedIds.map(notification_id =>
        local("/sync/mark-read", { method: "POST", body: JSON.stringify({ notification_id }) })
      ));
