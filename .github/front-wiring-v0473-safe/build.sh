...
set -euo pipefail
ROOT="$(pwd)"
PATCHDIR="$ROOT/.github/front-wiring-v0473-safe"
BASEBUILD="$ROOT/.github/backend-update-v0471/build.sh"
OUT="$ROOT/out"
TMP="${RUNNER_TEMP:-/tmp}/synthia-v0473-safe"
...
LAUNCHER="assets/synthia-start.html"
DEX="classes2.dex"
APK_NAME="Synthia-Phone-Selfhosted-v0.4.7.3-SAFE-FRONTEND.apk"
...
say "Rebuild exact working v0.4.7.1 baseline"
chmod +x "$BASEBUILD"
RUNNER_TEMP="$TMP/base-runner" "$BASEBUILD"
test -f "$OUT/$BASE_NAME"
cp "$OUT/$BASE_NAME" "$TMP/base-v0471.apk"
unzip -tq "$TMP/base-v0471.apk" >/dev/null

say "Acquire existing signing identity"
...
say "Extract only static frontend entries"
...
say "Register Synthia, Admin, Foundry and System status"
...
grep -q '/mcp/synthia/status' "$TMP/patch/$SHELL"
grep -q '/synthia/public' "$TMP/patch/$SHELL"
grep -q '/sync/status' "$TMP/patch/$SHELL"
! grep -q '/sync/control' "$TMP/patch/$SHELL"
! grep -q 'synthia-admin-button' "$TMP/patch/$SYNC"
! grep -q "id:'saf-btn'" "$TMP/patch/$FOUNDRY"

say "Patch APK frontend only"
rm -rf "$OUT" && mkdir -p "$OUT"
for entry in AndroidManifest.xml "$DEX" "$SERVER" "$LAUNCHER"; do
  unzip -p "$TMP/base-v0471.apk" "$entry" | sha256sum | awk '{print $1}' > "$TMP/base-$(echo "$entry" | tr '/.' '__').sha256"
done
cp "$TMP/base-v0471.apk" "$TMP/patched-unsigned.apk"
zip -q -d "$TMP/patched-unsigned.apk" "$SHELL" "$SYNC" "$FOUNDRY" || true
zip -q -d "$TMP/patched-unsigned.apk" 'META-INF/*' || true
(cd "$TMP/patch" && zip -q "$TMP/patched-unsigned.apk" "$SHELL" "$SYNC" "$FOUNDRY")
...
say "Prove native/backend layer is byte-identical to working v0.4.7.1"
for entry in AndroidManifest.xml "$DEX" "$SERVER" "$LAUNCHER"; do
  unzip -p "$OUT/$APK_NAME" "$entry" | sha256sum | awk '{print $1}' > "$TMP/new-$(echo "$entry" | tr '/.' '__').sha256"
  cmp "$TMP/base-$(echo "$entry" | tr '/.' '__').sha256" "$TMP/new-$(echo "$entry" | tr '/.' '__').sha256"
done

say "Verify only three frontend payload entries changed"
...
for allowed in "$SHELL" "$SYNC" "$FOUNDRY"; do rm -f "$TMP/before/$allowed" "$TMP/after/$allowed"; done
...
jq -n \
  --arg release "Synthia Phone Selfhosted v0.4.7.3 — Safe Frontend" \
  --arg ancestor "$BASE_NAME" \
  --arg cert "$new_cert" \
  --arg shell_sha256 "$shell_sha" \
  --argjson size "$size" \
  '{release:$release,ancestor:$ancestor,manifest_identical:true,classes2_identical:true,server_js_identical:true,launcher_identical:true,linux_backend_unchanged:true,signing_certificate_sha256:$cert,size_bytes:$size,repairs:["register Synthia in app tray","register Admin Inbox in app tray","register Artifact Foundry in app tray","remove global floating Admin UI","remove floating Foundry launcher","implement Settings > System using existing v0.4.7.1 endpoints","wire chat to existing /synthia/public route","wire Pure Synthia status to existing /mcp/synthia/status route"],changed_entries:["mobile-linux/assets/synthia-local.js","mobile-linux/assets/synthia-sync-connector.js","mobile-linux/assets/synthia-artifact-foundry.js"],upgrade_mode:"static APK frontend overlay; no installed-backend refresh and no native mutation",shell_sha256:$shell_sha256}' > "$OUT/VERIFICATION.json"
printf '\nBuilt: %s\n' "$OUT/$APK_NAME"
