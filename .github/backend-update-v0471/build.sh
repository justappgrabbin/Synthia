...
set -euo pipefail
ROOT="$(pwd)"
PATCHDIR="$ROOT/.github/backend-update-v0471"
...
jq -e '.endpoint and .file_endpoint and .workspace_id and .workspace_token' "$TMP/input/phone-sync-config.json" >/dev/null
...
if(!store)store=first([/\bpassword\s*[:=]\s*([^\s]+)/i]); if(!key)key=store;
...
zip -q -d "$TMP/patched-unsigned.apk" "$INDEX" "$SYNC" "$FOUNDRY" "$SERVER" "$PHONECFG" || true
zip -q -d "$TMP/patched-unsigned.apk" 'META-INF/*' || true
(cd "$TMP/final" && zip -q "$TMP/patched-unsigned.apk" "$INDEX" "$SYNC" "$FOUNDRY" "$SERVER" "$PHONECFG")
...
say "Verify v0.4.7 UI/Foundry are unchanged and only backend spine differs"
...
unzip -p "$OUT/$APK_NAME" "$PHONECFG" | jq -e '.workspace_id and .workspace_token' >/dev/null

# Compare every payload entry to reconstructed current 4.7. The only new differences
# allowed relative to 4.7 are SYNC, SERVER, and PHONECFG.
...
rm -f "$TMP/ref47/$SYNC" "$TMP/after/$SYNC"
rm -f "$TMP/ref47/$SERVER" "$TMP/after/$SERVER"
rm -f "$TMP/ref47/$PHONECFG" "$TMP/after/$PHONECFG"
if ! diff -qr "$TMP/ref47" "$TMP/after" > "$TMP/diff.txt"; then cat "$TMP/diff.txt"; exit 1; fi
...
jq -n \
  ...
  --argjson size "$size" \
  '{release:$release,ancestor:$ancestor,manifest_identical:true,signing_certificate_sha256:$cert,size_bytes:$size,new_changes_relative_to_v047:["file-origin sync requests now target http://127.0.0.1:3000","real /sync backend routes restored","phone sync/update configuration installed","background update polling enabled","downloaded update files SHA-256 verified before inbox admission"],unchanged_from_v047:["visible v0.4.7 shell","Artifact Foundry","native Android runtime","bundled Linux rootfs"],changed_entries_relative_to_v047:["synthia-sync-connector.js","server.js","config/phone-sync.json"],connector_sha256:$connector_sha256,server_sha256:$server_sha256}' > "$OUT/VERIFICATION.json"
printf '\nBuilt: %s\n' "$OUT/$APK_NAME"
